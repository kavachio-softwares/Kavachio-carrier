...
column_mapping  {output_sheet: {output_col: rule}}, rule is one of:
   {"kind": "copy",         "source": "<input col>",
                            "default": <literal>}   # optional — Appendix 2 §2.7/§2.8
                            # COALESCE: used only when the source cell is blank
                            # (None / NaN / '' / whitespace-only). Omit it and the
...
def eval_rule(rule: Any, in_row: dict, out_row: dict, constants: dict) -> Any:
    ...
    kind = rule.get("kind")
    if kind == "copy":
        v = in_row.get(rule.get("source"))
        # Appendix 2 §2.7 / §2.8 — COALESCE the blank to the agreed default.
        #
        ...
        #     txn code     -> COALESCE(..., 'UNK')
        #
        # §2.8 is explicit that this belongs in the OUTPUT — "Null transaction
        # codes are not permitted in output. 'UNK' is the required default." The
        # warehouse already applies the same defaults at ingest; without this the
        # delivered file and the sub-ledger disagree about the same row.
        #
        # `_is_blank` IS the NULLIF(TRIM(x), '') half: None, NaN, '' and a
        # whitespace-only cell all count as missing.
        #
        # Carried as an OPTIONAL key on the existing `copy` rule rather than a new
        # kind, deliberately: a rule without "default" behaves exactly as before,
        # and resolve_landing_cell's Fix write-back keeps working (it dispatches on
        # kind == "copy", so a new kind would make these fields uneditable).
        #
        # The default does NOT hide the problem — it makes it legible. Library
        # rules 50 and 53 flag the defaulted VALUES ('Unknown', 'UNK'), so the
        # broker sees a filled cell AND a Critical exception saying we filled it.
        # A blank cell is ambiguous; a marked default is not.
        if _is_blank(v) and "default" in rule:
            return rule["default"]
        return v
    if kind == "const":
        val = rule.get("value")
