...
# Code-defined templates that ship WITH their compiler builder (in rule_compiler)
# instead of a shared-DB `rule_template` row. This keeps a new rule type fully
# SELF-CONTAINED in code: the branch that adds the builder also adds its catalog
# entry here (so assert_builders_cover_catalog stays balanced), and it touches
# nothing in the shared DB — so a branch WITHOUT this code is completely
# unaffected (no orphaned catalog row can crash another developer's upload flow).
# A real DB row of the same name, if ever seeded, takes precedence (see below).
_CODE_SUPPLEMENTAL_TEMPLATES = [
    {
        "name": "zip_state_consistency",
        "engine": "custom",
        "rule_type": "zip_state_consistency",
        "required": ["zip_field", "state_field"],
        # `country_field` is OPTIONAL: when the template carries a country column
        # the check dispatches to that country's postal reference (US ZIP, Canadian
        # FSA, UK outward code); without it a value passes if it is valid in any
        # supported country. `countries` optionally narrows the set.
        "field_params": {"field_keys": ["zip_field", "state_field", "country_field"],
                         "include_scope": True},
        "description": ("zip_field must be a valid postal code for state_field on "
                        "the same row, in the country given by country_field when "
                        "present (offline data-quality check; US / CA / GB)."),
    },
    {
        "name": "state_validity",
        "engine": "custom",
        "rule_type": "state_validity",
        "required": ["state_field"],
        "field_params": {"field_keys": ["state_field", "country_field"],
                         "include_scope": True},
        "description": ("state_field must be a real state / province / region — a "
                        "code or full name present in the postal reference for the "
                        "row's country (offline data-quality check; US / CA / GB)."),
    },
    {
        "name": "currency_country_consistency",
        "engine": "custom",
        "rule_type": "currency_country_consistency",
        "required": ["currency_field", "country_field"],
        "field_params": {"field_keys": ["currency_field", "country_field"],
                         "include_scope": True},
        "description": ("currency_field must be a currency that is legal tender "
                        "in country_field on the same row (offline data-quality "
                        "check)."),
    },
]
...
def _build_catalog() -> dict:
    cat: dict[str, dict] = {}
    rows = list(get_template_rows())
    db_names = {r.get("name") for r in rows}
    # DB rows first; code-supplemental templates fill in only what the DB lacks so a
    # seeded row always wins and the same name is never registered twice.
    rows += [t for t in _CODE_SUPPLEMENTAL_TEMPLATES if t.get("name") not in db_names]
    for row in rows:
        name = row.get("name")
        if not name:
            continue
...
def malformed_field_refs(spec: dict, params: dict) -> list:
    """Field-valued params that are PRESENT but are not a column NAME —
    `{"right_field": 0.28}` where the model wrote the factor into the operand
    slot, `{"field": ["A","B"]}` where a single column was required.

    `_make_fields_fn` keeps only `isinstance(v, str)` values, because a non-string
    cannot be looked up in the template's field set — but that silently turns a
    broken reference into NO reference, so the field-existence gate never sees it
    and the rule sails through to the compiler. Read from the SAME declarative
    `field_params` the extractor is built from, so a new template is covered the
    moment its row declares its field keys."""
    fp = (spec or {}).get("field_params") or {}
    bad = []
    ...
    for k in fp.get("field_keys") or []:
        check(k, params.get(k))
    for k in fp.get("list_field_keys") or []:
        v = params.get(k)
        if isinstance(v, (list, tuple)):
            for item in v:
                check(k, item)
            ...
            check(k, v)
    for k in [fp.get("condition_list_key")] if fp.get("condition_list_key") else []:
        for c in (params.get(k) or []):
            if isinstance(c, dict):
                check(f"{k}.field", c.get("field"))
    for path in fp.get("nested_field_paths") or []:
        cur = params
        for part in path.split("."):
            cur = cur.get(part) if isinstance(cur, dict) else None
        check(path, cur)
...
def validate_ir(ir: dict, field_names: set[str]) -> tuple[bool, str | None]:
    ...
    # A field slot holding something that is not a column NAME is broken at the
    # source — it can never be "mapped" to a template column, so it is rejected
    # outright (this reason deliberately does NOT read "not in output template",
    # which is the caller's cue to keep the rule and compile it anyway).
    bad_refs = malformed_field_refs(spec, params)
    if bad_refs:
        return False, f"field reference is not a column name: {', '.join(bad_refs)}"
