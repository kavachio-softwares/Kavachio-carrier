...
def extract_rule_intents(clauses, batch_size=None):
    ...
    if batch_size is None:
        over, est, limit = would_truncate(json.dumps(clauses, default=str), 0,
                                          ratio=_CALL2_OUTPUT_RATIO)
        if total > _CALL2_SINGLE_MAX or over:
            batch_size = _CALL2_AUTO_BATCH
            why = (f"{total} clauses > {_CALL2_SINGLE_MAX}"
                   if total > _CALL2_SINGLE_MAX
                   else f"estimated answer ~{est} tok > output budget {limit}")
            print(f"[Call 2] {why}; auto-chunking.")
