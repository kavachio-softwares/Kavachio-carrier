...
G5/G6 are not politeness. They are the SAME tests contract generation applies
...
# Mirrors rule_compiler's runtime matcher, so we can tell the admin when the rule
# ALREADY accepts what they typed and adding it would change nothing.
_FUZZY_THRESHOLD = float(os.getenv("KAVACHIO_ENUM_MATCH_THRESHOLD", "0.90"))
_MIN_FUZZY_LEN = int(os.getenv("KAVACHIO_ENUM_MIN_FUZZY_LEN", "4"))
...
def _similarity(a: str, b: str) -> float:
    ...
    compiled rule uses — DuckDB's jaro_winkler_similarity.

    This must not be approximated. difflib was used here originally, on the
    assumption that a request handler has no DuckDB; that was wrong (DuckDB is an
    in-process library, already a dependency of the validation runtime) and the
    two disagree badly. Real case: "CGL US Surplus Line Policies" against an
    accepted "US Surplus Lines Policies" scores 0.913 on difflib but 0.800 on
    jaro-winkler. The gate told an admin "the rule already matches this" and
    refused a variation the rule does NOT match — the one lie this whole module
    exists to avoid.

    Falls back to difflib only if DuckDB cannot be loaded at all.
    """
    global _JW_CON
    try:
        if _JW_CON is None:
            import duckdb
            _JW_CON = duckdb.connect()
        return float(_JW_CON.execute(
...
def _acronym_context(base, candidates) -> str:
    ...
    refused outright by G5 and never reaches this prompt.
    """
    lines = []
    for c in candidates:
...
def _word_evidence(base, candidates) -> str:
    ...
    out = ("COMPUTED WORD BREAKDOWN (containment only — it proves NOTHING by "
           ...
           "leaves the meaning unchanged, it is the same value. If they name a "
           "place, an organisation, a product line or any other distinct "
           "identifier, it is a DIFFERENT entity that merely begins with the same "
           "words — REJECT those.\n")
    if coded:
...
def _check_deterministic(spelling, *, template, field, base, existing,
    ...
    """Gates G1–G6 only — free, instant, and no model. Every one of these
    ...
    # ── G4 — the rule's fuzzy matching already covers it ─────────────────
    sn = _norm(clean)
    if len(sn) >= _MIN_FUZZY_LEN:
        for k in known:
            kn = _norm(k)
            if len(kn) < _MIN_FUZZY_LEN:
                continue
            if _similarity(sn, kn) >= _FUZZY_THRESHOLD:
                return _reject("fuzzy_equivalent",
                               f"The rule already matches this as a near-spelling "
                               f"of “{k}”. Adding it would not change anything.")

    # ── G5 — does it trace back to something the contract names? ─────────
    ...
    # ── G5b — a listed value with extra words bolted onto it ─────────────
        ...
        for b in base:
            ...
            if len(extra) > _MAX_EXTRA_WORDS:
                ...
                concern = (f"is “{b}” with “{' '.join(extra)}” added — extra words "
                           f"are how a LONGER, DIFFERENT name is built, so confirm "
                           f"it is the same entity and not a related one")
                concern_reason = (
                    f"“{clean}” is “{b}” with extra words added, which could not be "
                    f"confirmed as the same entity.")
                break

    # ── G6 — too generic to identify one value ───────────────────────────
...
def _ai_admit(spelling, *, base, field, clause_text, existing, pol, ai=None,
    ...
    prompt += (
        ...
        "CATEGORY as one listed value — an abbreviation, a dropped legal suffix, "
...
def _ai_admit_batch(spellings, *, base, field, clause_text, existing, pol,
    ...
    prompt += (
        ...
        "one listed value — an abbreviation, a dropped legal suffix, an initialism, "
