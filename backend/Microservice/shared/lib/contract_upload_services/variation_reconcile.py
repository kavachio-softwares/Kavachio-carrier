...
Per enum rule (value_in_set / value_not_in_set), all best-effort:
  ...
  3. FUZZY pre-check (DuckDB jaro_winkler, the same metric the compiled SQL uses):
     a data value whose best similarity to any rule value is >= MATCH is ALREADY
     handled by the existing query — it is NOT a candidate.
  4. For the remaining candidates (values the query currently does NOT match) ask
...
Env knobs:
  ...
  KAVACHIO_ENUM_MATCH_THRESHOLD  fuzzy match threshold (default 0.90, mirrors the compiler)
...
_MATCH = float(os.getenv("KAVACHIO_ENUM_MATCH_THRESHOLD", "0.90"))
...
def _best_sim_per_value(con, distinct_vals: list, rule_vals: list) -> dict:
    """{data_value: best jaro_winkler similarity to any rule value} in ONE query,
    using DuckDB's jaro_winkler_similarity on normalized tokens."""
    dvals, dseen = [], set()
    for d in distinct_vals:
        n = _norm(d)
        if n and n not in dseen:
            dseen.add(n)
            dvals.append((d, n))
    rtoks = sorted({_norm(v) for v in rule_vals if _norm(v)})
    if not dvals or not rtoks:
        return {}
    d_sql = ", ".join(f"({_lit(d)}, {_lit(n)})" for d, n in dvals)
    r_sql = ", ".join(f"({_lit(t)})" for t in rtoks)
    q = (f"SELECT d.disp, max(jaro_winkler_similarity(d.nrm, r.nrm)) AS s "
         f"FROM (VALUES {d_sql}) AS d(disp, nrm), (VALUES {r_sql}) AS r(nrm) "
         f"GROUP BY d.disp")
    try:
        return {row[0]: float(row[1]) for row in con.execute(q).fetchall()}
    except Exception:
        return {}
...
def reconcile(con, tables: dict, rules: list, *, session=None, ai=None,
    ...
    for rule in rules or []:
        try:
            ...
            # FUZZY pre-check: a data value already >= MATCH to a rule value is
            # handled by the existing SQL → not a candidate. Candidates are the
            # data values the query currently does NOT match.
            sims = _best_sim_per_value(con, distinct, vvals)
            candidates = [d for d in distinct
                          if _norm(d) not in known and sims.get(d, 0.0) < _MATCH]
            if not candidates:
                continue
