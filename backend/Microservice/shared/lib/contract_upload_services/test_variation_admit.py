...
CASES = [
    ...
    ("Palms Specialty Ins.", "value_in_set", BASE, "fuzzy_equivalent",
     "'Palms Specialty' is already accepted and this sits at >= 0.90 similarity "
     "to it, so the compiled rule already matches it — reporting success would "
     "be a lie"),
    ...
    # jaro-winkler weights a shared PREFIX heavily, so this scores above 0.90
    # against "Palms Insurance Company, Limited" — the rule genuinely already
    # matches it, and "already covered" is the more useful answer than "ambiguous".
    ("Palms Insurance", "value_in_set", BASE, "fuzzy_equivalent",
     "already fuzzy-matched by the compiled rule"),
    ...
    ("Palms Specialty Insurance Co., Inc", "value_in_set", BASE, "fuzzy_equivalent",
     "the compiled rule already matches this at >= 0.90 similarity"),
    ...
    # THE WRONG-SIMILARITY-FUNCTION BUG — found on live rule "Authorized Policy
    # Type". G4 must use the SAME function the compiled rule uses (DuckDB's
    # jaro_winkler_similarity), not difflib. For this pair difflib scores 0.913
    # and jaro-winkler 0.800, so difflib refused a variation the rule does NOT
    # match, telling the admin it was "already matched".
    ("CGL US Surplus Line Policies", "value_in_set",
     ["Commercial General Liability US Surplus Lines Policies"], "accepted",
     "jaro-winkler 0.800 against the closest accepted spelling — NOT covered"),
    ("US Surplus Lines Policie", "value_in_set", ["US Surplus Lines Policies"],
     "fuzzy_equivalent", "a genuine near-spelling: jaro-winkler above 0.90"),
