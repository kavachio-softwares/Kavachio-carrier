...
def _page_text_with_ocr_fallback(page) -> str:
    ...
    ``get_text("text")`` reads the PDF's embedded text layer. A scanned page has
    no text objects, so it comes back effectively empty. In that case we render
    the page and OCR it via PyMuPDF's built-in Tesseract bridge
    (``get_textpage_ocr``), so image-only contracts still yield readable text
    instead of tripping the INSUFFICIENT_TEXT gate.
