...
def ir_batch_response_schema() -> dict:
    """JSON-schema for structured output, generated from the catalog so the
    schema and the verifier never drift. The high-value constraint is the
    `template` enum (templates ∪ null) — it stops invented template names. The
    per-template `params` shape is validated deterministically by validate_ir,
    so it is left as a generic object here."""
    return {
        "type": "object",
        "properties": {
            "results": {
...
1) Decide if it is RULE-BEARING — i.e. it can become a data check on a policy
   ...
   NOT rule-bearing: definitions, framing, governance, commercial relationship,
   claims handling, signatures; and PURE PROCESS obligations that name NO value a
   bordereau column would carry — e.g. who may sign, panel / TPA / vendor usage,
   the mechanics of a notification / submission workflow, reporting/accounting
   deadlines ("submit a report by the 15th"). These have no testable field.
...
2) If rule-bearing, extract one or more INTENTS describing WHAT to check —
   ...
   - operator: one of [{_INTENT_OPERATORS}].
       ...
       a PROGRAM / CONTRACT PERIOD BOUNDARY DATE (per DECISIVE TEST (f)) →
         ...
         Emit ONE date_bound intent PER stated boundary (a clause giving BOTH an
         inception and an expiration date yields TWO intents — one lower bound on
         the start date, one upper bound on the end date). Do NOT use date_relation
         here (that is column-vs-column); the boundary is a FIXED contract date.
...
PERMISSIVELY now:
  ...
  • Still respect the hard guards: never bind a value onto a structural KEY /
    ...
    policy period), and keep the correct value KIND.
